...
<umb:Pane ID="installation" runat="server" Visible="false">
    
    <div class="cour-dash-install">
    <h4 class="alert alert-error">Oops! The Courier database is not installed</h4>
    <p>
        We've detected that the Courier database tables are not installed. This may happen if the Courier package was not installed directly on this
        environment. Not to worry! Just click the button below to have the database tables installed.
    </p>
        <div>
            <asp:Button runat="server" Text="Install database tables" OnClick="OnInstallDatabaseTablesClick"  />
        </div>
    </div>
</umb:Pane>
